...
func TestDiff(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("err creating second file\n%v", err)
	}
	defer f2.Close()

	if err := f2.Chmod(0777); err != nil {
		t.Fatalf("eff chmodding file 2\n%v", err)
	}

	if _, err := f2.WriteString("12"); err != nil {
		t.Fatalf("err writing to second file\n%v", err)
	}
	...
	if err != nil {
		t.Fatalf("err reading patch file\n%v", err)
	}
...
func TestCombineDiff(t *testing.T) {
	...
	if err := f3.Chmod(0777); err != nil {
		t.Fatalf("eff chmodding file 3\n%v", err)
	}

	if _, err := f3.WriteString("123"); err != nil {
		t.Fatalf("err writing to third file\n%v", err)
	}
